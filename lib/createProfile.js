...
function drawAnimatedProgressBar(ctx, x, y, width, height, progress, label) {
  ...
  // Background bar dengan efek glassmorphism
  ...
  // Progress bar dengan gradien yang lebih menarik
  ...
  // Animasi stripe yang lebih halus
  ctx.save();
  ctx.clip();
  ...
  // Efek cahaya di ujung progress bar
  ...
  // Teks persentase dengan styling lebih baik
...
async function createProfileCard({
  ...
  level = '99',
  level_cache = 75,
  money = '∞', // Added money field with default value
  ...
  try {
    ...
    // Overlay gelap dengan gradien yang lebih halus
    ...
    // Efek aksen dengan gradien yang lebih menarik
    ctx.globalAlpha = 0.1;
    ...
    // Card dengan efek glassmorphism yang lebih baik
    const cardX = 50;
    const cardY = 180;
    ...
    // Profile picture dengan efek glow yang lebih menarik
    const pfpSize = 180;
    const pfpX = cardX + 60;
    const pfpY = cardY - (pfpSize / 2);
    
    // Efek glow yang lebih kompleks
    const glowGradient = ctx.createRadialGradient(
      pfpX + pfpSize / 2, pfpY + pfpSize / 2, pfpSize / 2 - 5, 
      pfpX + pfpSize / 2, pfpY + pfpSize / 2, pfpSize / 2 + 15
    );
    ...
    // Gambar profile dengan clipping
    ctx.save();
    ctx.beginPath();
    ...
    // Border profile picture dengan efek glow
    ...
    // Informasi pengguna dengan spacing yang lebih baik
    const textStartX = pfpX + pfpSize + 60;
    let currentY = cardY + 80;
    
    // Nama dengan gradien yang lebih menarik dan auto-resize
    ...
    // Garis pemisah dengan gradien yang lebih menarik
    currentY += 40;
    ...
    // Detail informasi dengan spacing yang lebih baik
    currentY += 60;
    const iconSize = 24;
    const labelColor = '#a0a0a0';
    const valueColor = '#ffffff';
    
    // Tambahkan field money ke dalam array details
    const details = [
      { icon: '\uf007', label: 'User ID', value: id },
      { icon: '\uf3a5', label: 'Limit', value: limit },
      { icon: '\uf51e', label: 'Money', value: money }, // Added money field with money icon
      { icon: '\uf521', label: 'Role', value: role },
      { icon: '\uf005', label: 'Level', value: level }
    ];

    // Hitung lebar maksimum label untuk alignment yang lebih baik
    ...
    // Render setiap detail dengan styling yang lebih baik
    details.forEach((d, i) => {
      const yPos = currentY + (i * 40); // Mengurangi spacing dari 45 menjadi 40
      ...
      // Value dengan styling yang lebih baik
    ...
    // Progress bar dengan positioning yang lebih baik
    // Posisi progress bar disesuaikan agar tidak menimpa informasi level
    const lastDetailY = currentY + ((details.length - 1) * 40);
    const barY = lastDetailY + 60; // Menambahkan jarak setelah detail terakhir
    const barWidth = cardWidth - 120;
    const barHeight = 30;
    const barX = cardX + 60;
    drawAnimatedProgressBar(ctx, barX, barY, barWidth, barHeight, level_cache, 'Level Progress');
    
    // Dekorasi tambahan dengan efek glow
    ctx.strokeStyle = 'rgba(0, 255, 255, 0.2)';
    ctx.lineWidth = 1;
    
    // Garis dekorasi di pojok kiri atas
    ctx.beginPath();
    ctx.moveTo(cardX + 30, cardY + 30);
    ctx.lineTo(cardX + 100, cardY + 30);
    ctx.stroke();
    
    // Garis dekorasi di pojok kanan bawah
    ctx.beginPath();
    ctx.moveTo(cardX + cardWidth - 100, cardY + cardHeight - 30);
    ctx.lineTo(cardX + cardWidth - 30, cardY + cardHeight - 30);
    ctx.stroke();
    
    // Garis dekorasi vertikal di pojok kiri
    ctx.beginPath();
    ctx.moveTo(cardX + 30, cardY + 50);
    ctx.lineTo(cardX + 30, cardY + 80);
    ctx.stroke();
    
    // Garis dekorasi vertikal di pojok kanan
    ctx.beginPath();
    ctx.moveTo(cardX + cardWidth - 30, cardY + 50);
    ctx.lineTo(cardX + cardWidth - 30, cardY + 80);
    ctx.stroke();

    // Titik-titik dekoratif dengan efek glow
    const dotPositions = [
    ...
    // Watermark dengan styling yang lebih baik
